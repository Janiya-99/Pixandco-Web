...
export function RollingButton({ href, children, variant = "light", className }: { href: string; children: React.ReactNode; variant?: "light" | "outline"; className?: string }) {
  return (
    <Link href={href} className={cn("group focus-ring inline-flex min-h-12 items-center gap-8 rounded-[8px] border px-5 text-xs font-medium uppercase tracking-[.12em] transition-colors", variant === "light" ? "border-white bg-white !text-black hover:bg-[#dcdcd8]" : "border-white/20 hover:border-white/50 text-white", className)}>
